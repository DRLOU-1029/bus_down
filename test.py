"""
单视频乘客检测、ReID与下车计数脚本 (最终优化版 + 最终位置显示)
...
class SingleVideoProcessor:
    ...
    def __init__(self, yolo_model, reid_model_path, reid_num_classes):
        ...
        # <--- 新增/修改: 用于跟踪ID最终位置的属性 ---
        self.disappeared_tracks_final_info = {} # 存储 {global_id: {'box': (x1,y1,x2,y2), 'label': '...'}}
        self.last_known_track_info = {}         # 存储 {track_id: {'box': ..., 'global_id': ...}}
        self.previous_frame_track_ids = set()   # 存储上一帧出现过的 track_id
        # <--- 新增/修改结束 ---
    ...
    def run(self, video_path_str: str, save_output: bool = True):
        ...
        if save_output:
            ...
            output_path = output_dir / f"{video_path.stem}_final_optimized_with_last_pos.mp4" # <--- 新增/修改: 修改输出文件名
        ...
        while cap.isOpened():
            ...
            # <--- 新增/修改: 检测并存储消失的ID ---
            current_track_ids = set(boxes.id.cpu().numpy().astype(int)) if boxes.id is not None else set()
            if frame_idx > 1:
                disappeared_track_ids = self.previous_frame_track_ids - current_track_ids
                for track_id in disappeared_track_ids:
                    if track_id in self.last_known_track_info:
                        info = self.last_known_track_info[track_id]
                        gid = info.get('global_id')
                        if gid is not None:
                            self.disappeared_tracks_final_info[gid] = {
                                'box': info['box'],
                                'label': f"Last Pos ID:{gid}"
                            }
            # <--- 新增/修改结束 ---
            ...
            temp_current_track_info = {} # <--- 新增/修改: 临时存储当前帧信息
            ...
            if boxes.id is not None:
                for box_coords, track_id in zip(boxes.xyxy.cpu().numpy(), boxes.id.cpu().numpy()):
                    ...
                    # <--- 新增/修改: 如果一个消失的ID重新出现，则从最终位置字典中移除 ---
                    if global_person_id is not None and global_person_id in self.disappeared_tracks_final_info:
                        del self.disappeared_tracks_final_info[global_person_id]
                    # <--- 新增/修改结束 ---
                    ...
                    # <--- 新增/修改: 实时更新当前帧所有活跃ID的信息 ---
                    temp_current_track_info[track_id] = {
                        'box': box_coords,
                        'global_id': global_person_id
                    }
                    # <--- 新增/修改结束 ---

            # <--- 新增/修改: 绘制所有已消失ID的最终位置框 ---
            for gid, info in self.disappeared_tracks_final_info.items():
                ...
                cv2.rectangle(vis_frame, (x1, y1 - th - 4), (x1 + tw, y1), color, -1)
                cv2.putText(vis_frame, label, (x1, y1 - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
            # <--- 新增/修改结束 ---
            ...
            self.last_known_track_info = temp_current_track_info
            # <--- 新增/修改结束 ---
...
def main():
    ...
    # --- 模型配置 (请确保路径和参数正确) ---
    YOLO_MODEL = r"D:\AI\DeepSORT_YOLOv8_Pytorch\best3.pt" # 替换为您的YOLOv8模型路径
