"""
单视频乘客检测、ReID与下车计数脚本 (最终自动化版 v2.1 - 带调试可视化)

功能:
- 对指定的单个视频文件进行乘客检测、追踪和ReID。
- (新) 自动分析视频，通过聚类追踪消失点来定义“下车区域”。
- (新) 在聚类前，会生成一张包含所有消失点的调试图，用不同颜色区分有效/无效点。
- 继承了鲁棒的ID分配策略和运动逻辑优化。

主要改进 (v2.1):
1.  **调试可视化**: 在自动定义区域时，会先弹出一张分析图，显示所有被考虑的消失点。
    - **绿色点**: 有效的消失点 (轨迹够长、方向正确)，将用于聚类。
    - **红色点**: 无效的消失点 (被过滤掉)，方便用户调试参数。
2.  **区域定义更智能**: 继承v2的智能过滤器 (过滤短轨迹和错误方向的轨迹)。
3.  **完全自动化**: 无需人工划线或选择。
"""
import cv2
import numpy as np
...
# ==============================================================================
# ReID 模型占位符 (Placeholder)
# ==============================================================================
# !!重要!! 如果您有自己的 torchreid_model.py 文件, 请删除下面的类并取消注释下一行
# from torchreid_model import RealReIDModel
class RealReIDModel:
    """这是一个ReID模型的占位符实现。"""
    def __init__(self, model_path, num_classes, device='cpu'):
        print("="*60 + "\n警告: 正在使用 ReID 模型占位符 (Placeholder)。\n" + "="*60)
    def extract_feature(self, crop_image):
        """生成一个随机的、归一化的512维特征向量作为占位符。"""
        random_feature = np.random.rand(512).astype(np.float32)
        norm = np.linalg.norm(random_feature)
        return random_feature / norm if norm > 0 else random_feature
...
def define_disembarkation_zone(video_path_str: str, yolo_model: YOLO, analysis_frames: int = 500,
    ...
    """
    通过分析视频中追踪ID的消失点来自动定义下车区域, 并在聚类前提供可视化调试。
    """
    print(f"\n--- 自动定义下车区域 (智能过滤版 + 调试可视化) ---")
    ...
    for i in range(frames_to_process):
        ...
        if results[0].boxes.id is not None:
            for box, track_id in zip(results[0].boxes.xyxy.cpu().numpy(), results[0].boxes.id.cpu().numpy()):
                ...
                center = ((int(box[0]) + int(box[2])) // 2, int(box[3]))
    ...
    for tid, history in track_histories.items():
        ...
        if is_valid:
            valid_points.append(end_pos)
        else:
            invalid_points.append(end_pos)
    ...
    for point in invalid_points:
        cv2.circle(vis_debug_frame, point, 4, (0, 0, 255), -1)  # 红色: 无效点
    for point in valid_points:
        cv2.circle(vis_debug_frame, point, 5, (0, 255, 0), -1)  # 绿色: 有效点
...
class SingleVideoProcessor:
    ...
    def run(self, video_path_str: str, save_output: bool = True):
        ...
        self.disembarkation_zone = define_disembarkation_zone(video_path_str, self.yolo_model, analysis_frames=600, min_track_length=20, exit_direction_vector=(-1, 0))
        ...
        if save_output:
            ...
            output_path = output_dir / f"{video_path.stem}_auto_zone_v2_1.mp4"
...
def main():
    print(f"\n{'='*60}\n单视频乘客检测、ReID与下车计数脚本 (v2.1 - 带调试可视化)\n{'='*60}")
